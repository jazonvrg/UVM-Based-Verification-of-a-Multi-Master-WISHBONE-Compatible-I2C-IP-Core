class enable_empty_tx_fifo_test extends uart_base_test;
	`uvm_component_utils(enable_empty_tx_fifo_test)
	...
	function new(string name = "enable_empty_tx_fifo_test", uvm_component parent);
	...
	virtual task run_process();
		calc_divisor(cfg);
		regmodel.IER.write(status, {27'h0, 1'b0, 1'b0, 1'b0, 1'b1, 1'b0});
		if (cfg.ovsmpl == uart_configuration::X16) begin
		...
		for(int i = 1; i <= n; i = i + 1) begin
			...
			regmodel.FSR.read(status,rdata);
			if (i > 1) env.scb.compare(ahb_vif.interrupt, 1'b0);
			else env.scb.compare(ahb_vif.interrupt, 1'b1);
		end
		// empty_status = 1
		env.scb.selection = 2.1;
		if (cfg.parity_mode != uart_configuration::NONE) begin
