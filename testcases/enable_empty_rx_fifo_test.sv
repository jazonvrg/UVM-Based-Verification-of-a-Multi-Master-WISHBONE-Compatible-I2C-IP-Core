class enable_empty_rx_fifo_test extends uart_base_test;
	`uvm_component_utils(enable_empty_rx_fifo_test)
	...
	function new(string name = "enable_empty_rx_fifo_test", uvm_component parent);
	...
	virtual task run_process();
		calc_divisor(cfg);
		regmodel.IER.write(status, {27'h0, 1'b0, 1'b1, 1'b0, 1'b0, 1'b0});
		if (cfg.ovsmpl == uart_configuration::X16) begin
